...
def _read_csv(path: Path, *, parse_dates: Sequence[str] | None = None, chunksize: int | None = None) -> pd.DataFrame:
    if chunksize:
        chunks: Iterator[pd.DataFrame] = pd.read_csv(path, parse_dates=list(parse_dates or []), chunksize=chunksize)
...
def parse_data_inventory_csv(path: Path) -> pd.DataFrame:
    df = _read_csv(path, parse_dates=["start_date", "end_date"]) if path.exists() else pd.DataFrame()
    return df
