...
def find_nearest_station(
    ...
    if nearest_station:
        console.print(
            f"[green]Found nearest station: {nearest_station} ({min_distance:.2f} km away)[/green]"
        )
...
def parse_geometry_input(geometry_input: str) -> Optional[tuple[float, float]]:
    ...
    if "," in geometry_input and not geometry_input.strip().startswith(
        ("{", "POINT", "POLYGON", "MULTI")
    ):
        try:
...
def resolve_station_id(
    ...
    if geometry:
        ...
        if coords:
            ...
            console.print(
                f"[yellow]Warning: Could not parse geometry '{geometry}', ignoring[/yellow]"
            )
            geometry = None
    ...
    try:
        ...
        try:
            ...
            raise argparse.ArgumentTypeError(
                f"Invalid date format: {date_str}. Use ISO format or YYYY-MM-DD"
            )
...
Examples:
  ...
  %(prog)s --geometry '{"type":"Point","coordinates":[6.127742,46.247519]}' --start 2024-01-01
    ...
    console.print(
        Panel(summary_table, title="[bold]Fetching Weather Data[/bold]", border_style="green")
    )
    ...
    try:
        ...
        if df.empty:
            console.print(
                f"[yellow]No data found for station {station_id} in the requested date range[/yellow]"
            )
            return 1
        ...
        console.print(
            f"[dim]Columns: {', '.join(df.columns[:5].tolist())}{'...' if len(df.columns) > 5 else ''}[/dim]"
        )
        ...
        if args.output:
            ...
            cfg = APIConfig()
            output_path = (
                cfg.artifacts_outputs_dir / f"weather_data_{station_id}_{args.granularity}.csv"
            )
